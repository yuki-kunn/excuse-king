<script lang="ts">
  ...
  // ★追加：AIの判定結果を保存する変数（これがカラじゃない時に結果画面を出します）
 ...
 const submitExcuse = async () => {
    ...
    try {
      ...
      // ★大改修：データベースの保存を待たずに、AIの返答が来たら「すぐ」画面を切り替えます！
      aiResult = evaluation;
      isSubmitting = false;

      // 2. その裏側で、ゆっくりデータベースに保存する（もし保存に失敗しても画面はストップしません）
      try {
        await addDoc(collection(db, 'posts'), {
          themeId: themeId,
          userId: user.displayName || "名無し",
          excuseText: excuseText,
        ...
        // もし保存に失敗したら、裏側（コンソール）にだけエラーを出しておきます
        console.error("データベース保存エラー（ルール設定を確認してください）:", dbError);
      }
...
<div class="post-container">
  ...
  {#if aiResult}
    <div class="result-box">
      ...
      <div class="action-buttons">
        <a href={shareUrl} target="_blank" rel="noopener noreferrer" class="btn share-btn">
          𝕏 でシェアする
        </a>
        <button class="btn back-to-list-btn" on:click={() => goto(`/themes/${themeId}`)}>
          みんなの言い訳を見る
        </button>
      </div>
    </div>
  ...
  {:else}
    ...
    <div class="input-area">
      ...
      <textarea 
        id="excuse"
        bind:value={excuseText} 
        placeholder="いや、これには深い海よりも深い訳がありまして..."
        rows="5"
...
<style>
  ...
  .input-label {
    display: block; font-weight: 900; font-size: 18px; margin-bottom: 12px; color: #222; text-shadow: 1px 1px 0px #4285F4;
  }
  textarea {
    width: 100%; box-sizing: border-box; padding: 16px; font-size: 16px; font-weight: bold; border: 4px solid #222;
    border-radius: 12px; resize: none; background-color: #fff; color: #222; box-shadow: 6px 6px 0px #222; font-family: inherit;
  }
